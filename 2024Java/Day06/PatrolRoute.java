...
public class PatrolRoute {
    ...
    List<List<Character>> readFile() throws IOException {
        ...
        while ((linebuf = input.readLine()) != null) {
            List<Character> row = linebuf.chars().mapToObj(l -> (char) l)
                    .collect(ArrayList::new, ArrayList::add, ArrayList::addAll);
            if (!linebuf.isEmpty() && linebuf.matches(".*[\\^>v<].*")) {
                guardPos = new Point2D(routeMap.size(),
                        linebuf.split("[\\^>v<]")[0].length());
            }
            if (!row.isEmpty()) {
                routeMap.add(row);
            }
    ...
    void patrol(Point2D curPos, List<List<Character>> routeMap) {
        ...
        while (Point2D.isValid(rows, cols, curPos)) {
            // Determine the face
            curFace = faceId
                    .get(routeMap.get(curPos.getKey()).get(curPos.getValue()));
            Point2D nextPos = new Point2D(dx[curFace] + curPos.getKey(),
                    dy[curFace] + curPos.getValue());
            if (Point2D.isValid(rows, cols, nextPos)) {
                if (routeMap.get(nextPos.getKey()).get(nextPos.getValue())
                        .equals('#')) {
                    curFace = (curFace + 1) % 4;
                    routeMap.get(curPos.getKey()).set(curPos.getValue(),
                            face[curFace]);
                } else {
                    routeMap.get(curPos.getKey()).set(curPos.getValue(), 'X');
                    curPos = nextPos.extractPosPoint2D();
                    routeMap.get(nextPos.getKey()).set(nextPos.getValue(),
                            face[curFace]);
                }
            } else {
                routeMap.get(curPos.getKey()).set(curPos.getValue(), 'X');
    ...
    boolean isPatrolCircle(Point2D curPos, List<List<Character>> routeMap,
        int rows, int cols) {
        boolean isCircle = false;
        // HashMap<Point2D, Entry<Boolean, Character>> passed = new HashMap<>();
        List<List<Entry<Boolean, Character>>> passed = routeMap.stream()
                .map(line -> line.stream().map(
                        cell -> (Entry<Boolean, Character>) new SimpleEntry<>(
                                false, '.'))
                        .collect(Collectors.toList()))
                .collect(Collectors.toList());

        // List<List<Entry<Boolean, Character>>> passed = Collections
        //         .nCopies(rows,
        //                 Collections.nCopies(cols,
        //                         (Entry<Boolean, Character>) (new SimpleEntry<>(
        //                                 false, '.'))))
        //         .stream()
        //         .map(list -> (List<Entry<Boolean, Character>>) (new ArrayList<>(
        //                 list)))
        //         .toList();
        // int times = 0;
        int curFace = faceId
        ...
        while (Point2D.isValid(rows, cols, curPos)) {
            ...
            if (curStatus.getKey()
                    && curStatus.getValue().equals(face[curFace])) {
                return true;
            }
            // Determine the face
            Point2D nextPos = new Point2D(dx[curFace] + curPos.getKey(),
    ...
    int threadTask(int startRow, int endRow, Point2D guardPos,
        List<List<Character>> routeMap, AtomicInteger count) {
        int distinctPlacementLocal = 0;
        for (int i = startRow; i < endRow; i++) {
            for (int j = 0; j < routeMap.get(i).size(); j++) {
                // count.compareAndSet(count.get(), count.get() + 1);
                count.incrementAndGet();
                if (i == (int) guardPos.getKey()
                        && j == (int) guardPos.getValue()
                        || routeMap.get(i).get(j).equals('#')) {
                    continue;
                }
                Character curCell = routeMap.get(i).get(j);
                routeMap.get(i).set(j, '#');
                if (isPatrolCircle(guardPos, routeMap, routeMap.size(),
                        routeMap.get(0).size())) {
    ...
    void Solution2()
        ...
        for (int t = 0; t < numThreads; t++) {
            ...
            int endRow = (t == numThreads - 1) ? routeMap.size()
                    : (t + 1) * rowsPerThread;
        ...
        // int lastChecked = 0, total = routeMap.size() *
        // routeMap.get(0).size();
        ...
        // System.out.println(
        // "Positions checked: " + checked + "/" + total + " (" + (checked *
        // 100.0 /
        // total) + "%)");
