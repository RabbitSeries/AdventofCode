...
const posList = await readFile('Day07/input.txt')
    .then(data => data.toString().trim())
    .then(line => line.split(','))
    .then(row => row.map(pos => parseInt(pos)))
...
function minAlignFuel(submarines: Map<number, number>,
    fuelCost: (distance: number) => number) {
    let minFuel: number = Infinity
    const [min, max] = submarines.keys()
        .reduce((minMax, b) =>
            [Math.min(minMax[0], b), Math.max(minMax[1], b)],
        [Infinity, -Infinity])
...
console.log(minAlignFuel(submarines, (distance: number) =>
