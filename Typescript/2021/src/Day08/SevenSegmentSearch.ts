...
function countUnique(data: Data) {
    return data.map(v => v.outputs.reduce((prev, o) => (o.length === 2
      || o.length === 3 || o.length === 4 || o.length === 7)
        ? prev + 1
        : prev, 0)).reduce((a, b) => a + b)
}
...
function solveInput(signals: Set<string>[], outputs: Set<string>[]) {
    ...
    // Each digit uses the following number of wires:
    //  6, 2, 5, 5, 4, 5, 6, 3, 7, 6.
    ...
    // Segment a: The digits 1 (2 segments) and 7 (3 segments)
    //  help identify the wire for segment a.
    segments[segmentIdx('a')] = digits[7]
        .difference(digits[1]).keys().toArray()[0]

    // Digit 9:
    // Segment g: Three digits use 6 wires: digits 0, 6, and 9.
    // Digit 9 can be distinguished among them by
    //  checking whose wires include the 4 segments of digit 4.
    // Once that’s done, digit 9’s segments and
    //  the 6_th segment can be identified by
    // excluding segment a and the segments from digit 4,
    //  leaving the last segment (segment g).
    const digit06 = []
    ...
    // Digits 0 and 6:
    // Segment e: Digits 0 and 6 can be distinguished by
    //  whether their segments include those of digit 1.
    ...
    segments[segmentIdx('e')] = digits[6]
        .difference(new Set([segments[segmentIdx('a')],
            segments[segmentIdx('g')], ...digits[4]]))
        .keys().take(1).toArray()[0]
    ...
    segments[segmentIdx('f')] = digits[1]
        .difference(new Set([segments[segmentIdx('c')]])).keys().toArray()[0]

    // Segments b and d: After excluding the known segments from digit 6,
    //  segments b and d remain.
    // They can be distinguished because digit 0 only contains segment b.
    segments[segmentIdx('d')] = digits[6]
        .difference(digits[0]).keys().toArray()[0]
    segments[segmentIdx('b')] = digits[4]
        .difference(new Set([segments[segmentIdx('d')], ...digits[1]]))
        .keys().toArray()[0]
