...
const lines = await readFile('Day05/input.txt')
    .then(data => data.toString().trim().split(EOL).map((line) => {
        const posList = line.match(/(\d+),(\d+).*?(\d+),(\d+)/)?.slice(1)
            .map(n => parseInt(n))
        return posList
            ? {
                x1: posList[0],
...
function count_overlap(cross_only: boolean) {
    const process_lines = cross_only
        ? lines.filter(m =>
            m.x1 === m.x2 || m.y1 === m.y2)
        : lines
    const overlap_zone = Array.from({ length: 1000 }, () =>
        Array.from({ length: 1000 }, () => 0))
    for (const line of process_lines) {
        if (line.x1 === line.x2) {
            for (let y = Math.min(line.y1, line.y2);
            ...
            for (let dx = 0; dx <= Math.abs(line.x2 - line.x1); dx++) {
                ...
                const row = Math.floor(line.y1 + tan * v_dx)
                overlap_zone[row][line.x1 + v_dx]++
            }
        }
    }
    return overlap_zone.reduce((prev, row) =>
        prev + row.reduce((prev, cnt) => cnt > 1 ? (prev + 1) : prev, 0), 0)
}
