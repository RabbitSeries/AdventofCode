...
const input = await readFile('Day03/input.txt').then(buf =>
    buf.toString().split(EOL))
const commonBit = (s: string) =>
    [...s].filter(c => c === '1').length >= s.length / 2 ? '1' : '0'
const reverse = (s: '0' | '1') => s === '0' ? '1' : '0'
const commons = Array(input[0].length).map((_, col) =>
    input.map(s => s.charAt(col)).toSorted().join('')).map(commonBit)
const leasts = commons.map(reverse) // default separated with a comma
console.log(`Part 1: ${
    parseInt(commons.join(''), 2) * parseInt(leasts.join(''), 2)}`)
function filterBy(isCommon = false) {
    let q = input.map((_, sID) => ({ sID, bID: 0 }))
    while (q.length > 1) {
        const bitSet = q.map(e => input[e.sID][e.bID]).join('')
        const criteria = isCommon
            ? reverse(commonBit(bitSet))
            : commonBit(bitSet)
