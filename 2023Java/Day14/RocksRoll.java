...
public class RocksRoll {
    ...
    String getHash() {
        return Platform.stream().map(list -> list.stream().map(c -> c.toString()).collect(Collectors.joining()))
                .collect(Collectors.joining());
    }

    Pair<Integer, Integer> tilt(int times) {
        for (int step = 0; step < 4; step++) {
            for (int i = (step < 2 ? 0 : row - 1); (step < 2 ? i < row : i >= 0); i += (step < 2 ? 1 : -1)) {
                for (int j = (step < 2 ? 0 : col - 1); (step < 2 ? j < col : j >= 0); j += (step < 2 ? 1 : -1)) {
                    if (Platform.get(i).get(j).equals('O')) {
    ...
    void readFile() throws IOException {
        ...
        while ((buf = input.readLine()) != null) {
            Platform.add(buf.chars().mapToObj(c -> (char) c).collect(Collectors.toCollection(ArrayList::new)));
        }
        input.close();
        row = Platform.size();
