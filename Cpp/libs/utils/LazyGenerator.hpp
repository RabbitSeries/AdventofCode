...
namespace utils {
    ...
    struct LazyGenerator: public std::ranges::view_interface<LazyGenerator<T>> {
        ...
        LazyGenerator& operator=( LazyGenerator other ) { std::swap( other.h, this->h ); }
        ...
        static LazyGenerator fromProvider( std::function<std::optional<T>()> f ) {
            std::optional<T> value = f();
            while ( value ) {
        ...
        struct iterator {
            ...
            T&& operator*() const { return std::move( h.promise().current ); }

            T* operator->() const { return std::addressof( h.promise().current ); }
