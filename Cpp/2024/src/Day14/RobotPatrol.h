...
class RobotPatrol: public ISolution {
    ...
    std::vector<robot> step( std::vector<robot> s, int cnt ) {
        for ( auto& rbt : s ) {
            rbt.curPos.first =
                ( rbt.curPos.first + ( WIDTH + rbt.vel.first ) * cnt ) % WIDTH;
            rbt.curPos.second =
                ( rbt.curPos.second + ( HEIGHT + rbt.vel.second ) * cnt ) % HEIGHT;
        }
        return s;
    }
    ...
    void readFile() {
        ...
        for ( std::string buf; getline( input, buf ); ) {
            ...
            if ( m.size() == 5 ) {
                specs.push_back(
                    robot( stoi( m[1] ), stoi( m[2] ), stoi( m[3] ), stoi( m[4] ) ) );
            }
        }
    }
    ...
    int centerVariance( int stepCnt, bool isFirst, int SCALE ) {
        int var = 0;
        for ( const auto& rbt : specs ) {
            var += abs( ( getter( rbt.curPos, isFirst ) +
                          ( SCALE + getter( rbt.vel, isFirst ) ) * stepCnt ) %
                            SCALE -
                        ( SCALE - 1 ) / 2 );
        }
        return var;
    ...
    int positiveMod( int a, int b ) { return ( b + ( a % b ) ) % b; }
    ...
    void Solution2() {
        int minDxX = *std::ranges::min_element(
            std::views::iota( 0, WIDTH ), {},
            std::bind( std::mem_fn( &RobotPatrol::centerVariance ), *this,
                       std::placeholders::_1, true, WIDTH ) );
        int minDyY = *std::ranges::min_element(
            std::views::iota( 0, HEIGHT ), {},
            std::bind( &RobotPatrol::centerVariance, this, std::placeholders::_1, false,
                       HEIGHT ) );
        printRes(
            2, positiveMod( reverseMod( WIDTH, HEIGHT ) * ( minDyY - minDxX ), HEIGHT ) *
                       WIDTH +
                   minDxX );
    }
};
