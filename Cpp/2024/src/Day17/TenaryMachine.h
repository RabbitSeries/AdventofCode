...
class TenaryMachine: public ISolution {
    ...
    void ( TenaryMachine::* fun[8] )( ull operand ) = {
        &TenaryMachine::adv, &TenaryMachine::bxl, &TenaryMachine::bst,
        &TenaryMachine::jnz, &TenaryMachine::bxc, &TenaryMachine::out,
        &TenaryMachine::bdv, &TenaryMachine::cdv };
    ...
    void adv( ull operand ) { ra = ra >> combo( operand ); }

    void bxl( ull operand ) { rb = rb ^ operand; }

    void bst( ull operand ) { rb = combo( operand ) % 8; }

    void jnz( ull operand ) { pc = operand; }

    void bxc( ull ) { rb = rb ^ rc; }

    void out( ull operand ) { outBuf += "," + std::to_string( combo( operand ) % 8 ); }

    void bdv( ull operand ) { rb = ra >> combo( operand ); }

    void cdv( ull operand ) { rc = ra >> combo( operand ); }
    ...
    void readFile() {
        ...
        for ( std::string buf; std::getline( input, buf ); ) {
            ...
            if ( distance( it, end_it ) == 1 ) {
                ...
                while ( it != end_it ) {
                    instruction.push_back( std::stoi( it->str() ) );
                    program += "," + it->str();
                    it++;
                }
            }
