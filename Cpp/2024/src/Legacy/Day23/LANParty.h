...
class LANParty: public ISolution {
    ...
    std::generator<std::set<std::string>> BronKerbosch1( std::set<std::string> nextR,
        ...
        while ( !P.empty() ) {
            ...
            std::set_intersection( LANNetwork.at( v ).begin(), LANNetwork.at( v ).end(),
                                   P.begin(), P.end(), back_inserter( nextP ) );
            std::set_intersection( LANNetwork.at( v ).begin(), LANNetwork.at( v ).end(),
                                   X.begin(), X.end(), back_inserter( nextX ) );
            nextR.insert( *P.begin() );
            for ( const std::set<std::string>& res :
                  BronKerbosch1( nextR, std::set( std::from_range_t{}, P ),
                                 std::set( std::from_range_t{}, nextX ) ) ) {
                co_yield res;
            }
            P.erase( P.begin() );
            X.insert( v );
            ...
            for ( auto& netB : LANNetwork.at( netA ) ) {
                for ( auto& netC : LANNetwork.at( netB ) ) {
                    if ( LANNetwork.at( netA ).contains( netC ) &&
                        ...
                        partyset.insert( std::ranges::fold_left( party, std::string(),
                                                                 std::plus<>{} ) );
                    }
                }
            }
        ...
        for ( std::set<std::string> cliq : BronKerbosch1( {}, P, {} ) ) {
            clique.emplace(
                ...
                std::ranges::fold_left(
                    cliq, "", []( std::string const& add, std::string const& v ) {
                        return add.empty() ? v : ( add + "," + v );
                    } ) );
        }
