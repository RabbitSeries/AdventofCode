...
class Calibration: public ISolution {
    ...
    void readFile() {
        ...
        for ( string&& buf : fileLinesStream( "Day07/input.txt" ) ) {
            auto nums =
                regexStream( R"(\d+)", buf ) |
                views::transform( []( const smatch& m ) { return stoull( m.str() ); } ) |
                ranges::to<vector<ull>>();
            numList.emplace_back( nums[0], vector( nums.begin() + 1, nums.end() ) );
        }
    }

    size_t dfsParse( size_t test,
                     std::vector<ull> const& nums,
                     const size_t curPos,
                     const size_t curRes,
                     const int options ) {
        if ( curPos == 0 ) {
            return ( curRes == 0 ? test : 0 );
        }
        if ( curRes >= nums[curPos - 1] ) {
            if ( dfsParse( test, nums, curPos - 1, curRes - nums[curPos - 1], options ) ==
                 test ) {
                return test;
            }
            if ( ( curRes % nums[curPos - 1] ) == 0 &&
                 dfsParse( test, nums, curPos - 1, curRes / nums[curPos - 1], options ) ==
                     test ) {
                return test;
            }
            if ( options == 3 ) {
    ...
    size_t parseWith( const int options ) {
        return std::ranges::fold_left(
            numList | std::views::transform( [options, this]( const K_V& k_v ) {
                return dfsParse( k_v.first, k_v.second, k_v.second.size(), k_v.first,
                                 options );
            } ),
            0ull, std::plus<>{} );
    }
    ...
    void Solution2() { printRes( 2, parseWith( 3 ) ); }
};
