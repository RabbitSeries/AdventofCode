...
class CheckLevelInOrder: public ISolution {
    ...
    bool checkOrder( std::vector<int> const& Levels,
                     bool desc = true,
                     size_t skip = -1 ) {
        bool res = true;
    ...
    void Solution1() {
        readFile();
        printRes( 1, std::ranges::count_if( LevelLists, [this]( auto const& Levels ) {
                      return checkOrder( Levels ) || checkOrder( Levels, false );
                  } ) );
    }

    void Solution2() {
        printRes( 2, std::ranges::count_if( LevelLists, [this]( auto const& Levels ) {
                      return checkOrder( Levels ) ||
                             checkOrder( Levels, false ) ||
