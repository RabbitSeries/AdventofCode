...
int main() {
#ifdef __linux__
    FILE* pipe = popen( R"(cloc . --include-ext=h,cc,cpp,hpp,c,java,py,js,ts,mjs --exclude-dir=.vscode,compileRuntime,JavaLibs,build --not-match-d="node_modules|dist|target" 2>exception.log)", "r" );
#else
    FILE* pipe = popen( R"(wsl -e cloc . --include-ext=h,cc,cpp,hpp,c,java,py,js,ts,mjs --exclude-dir=.vscode,compileRuntime,JavaLibs,build --not-match-d="node_modules|dist|target" 2>exception.log)", "r" );
#endif
    if ( !pipe ) {
